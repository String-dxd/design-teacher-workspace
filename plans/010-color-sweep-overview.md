...
feature files bypass that system** — they hardcode Tailwind's _default_ palette
...
1. **Dark-mode is a latent landmine.** The dark tokens exist in `styles.css` but no
   ...
   usages will _not_ flip when dark mode is switched on — they will render
...
> palette after a 60-file audit. The token _registration_ work (slate-3..12, violet,
...
- **PINK** → all Tailwind `pink-*` map to **crimson**. The radar-chart
  `colorScheme='pink'` whose literal is `#f26c47` is _coral, not pink_ → handled as
