...
| 027  | HDP colour & token hygiene — one parent-facing accent, kill hard-coded hex (`#c47565`/`#f26c47`) + Tailwind-default slate + `bg-white` + `lime-600` + rgba-shadow across pg-report-preview-dialog / holistic-reports.index / cycle-student-table                                  | P1                    | S–M    | —                                | DONE (branch `design/hdp-report-polish`, executed via `tfx:design`) — all off-token literals removed (grep-clean). Parent accent refined after AA re-check: decorative accent = `orange-11` text on white (4.51:1); primary actions/count badges = `twblue` primary (4.92:1) not `orange-9` fills (which fail AA at 2.97). Gates: tsc 81 (0 new) / vitest 98-24 / build 0 / eslint clean.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 028  | HDP component reuse & report-document polish — hand-rolled cards → local `DocCard` (documented CMP-1 waiver), parallel pill system → ui/Badge, `text-[10/11px]` → `text-xs`, raw `<button>` → ui/Button, `uppercase` removed (TYP-4); same-component heading/title capitalization | P1                    | M      | 027                              | DONE (executed via `tfx:design` + `tfx:evaluator`) — DocCard consolidates 6 copies (byte-identical chrome, avoids ui/Card `rounded-3xl` nested-card slop per SLP-4); pills/conduct → ui/Badge; write-route Alert conversion deliberately skipped (error banner is A11Y-11 focus-managed, must not carry `role=alert`). Evaluator round 1 FAIL on contrast → fixed (see decision record).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 029  | HDP copy & voice pass — Title Case → sentence case labels, drop casual "Good job!", consistent apostrophes, "Issued date"/"Acknowledge report"                                                                                                                                    | P2                    | S      | 027, 028                         | DONE (executed via `tfx:copy`) — landed in the same branch as 027/028. Sentence-case labels across both hub routes + glance tiles; removed "Good job!"; curly apostrophes; proper nouns (Values in Action, Parents Gateway) preserved.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

Status values: TODO | IN PROGRESS | DONE | BLOCKED (with one-line reason) |
REJECTED (with one-line rationale).

## Round 7 — HDP design-consistency pass (2026-07-13)

Scoped `/improve` over the HDP surfaces (report document, cycle hub, write, layout,
send/submit/PG-preview dialogs) at commit `3aa0371` (branch `design/hdp-report-polish`),
run against the design-system foundations in `src/styles.css` + a visual baseline captured
with agent-browser at 1440×900 (cycle hub, write page, layout editor, PG preview dialog,
plus Posts + Student Insights as the "target look" reference). One fresh-context audit agent
swept the code for off-token colour, hand-rolled components, spacing/type divergence, and
copy tells; every finding re-verified against the code by the advisor.

**Headline: the HDP surfaces read as a slightly different design language than the rest of
the app.** Two recurring patterns drive it, both already flagged as deferred work in earlier
rounds:

1. **Off-token colour** concentrated on the parent-facing / Parents-Gateway surfaces — the
   `pg-report-preview-dialog.tsx` (added this pull) abandons the token system: hard-coded
   salmon `#c47565` ×2, Tailwind-default slate throughout, `bg-white`, `lime-600`, arbitrary
   radii/font-sizes. The same parent accent is rendered three ways across the folder
   (`bg-orange-9` / `#c47565` / `#f26c47`) — no single source of truth. Round 3 already
   decided these coral/salmon hexes map to the Radix `orange` scale; plan **027** finishes it.
2. **Hand-rolled components the DS already ships** — cards as `rounded-xl border bg-card`
   divs, chips as `rounded-full` spans, banners built by hand — instead of `ui/Card` /
   `ui/Badge` / `ui/Alert`. This is the "Primitive-reuse opportunities (~80)" deferred item.
   Plan **028** composes them onto the primitives.

Plus a copy layer: Title Case vs sentence case mixed within pages, a casual "Good job!" in a
formal report, inconsistent apostrophes → plan **029** (`tfx:copy`).

- **Suggested order: 027 → 028 → 029.** 027 (colour) first so 028's component diffs aren't
  tangled with colour swaps; 029 (copy) last so it lands on cleaned-up markup. All three are
  executed via the `tfx:design` / `tfx:copy` skills on this branch, browser-validated with
  agent-browser and iterated.
- **Baseline gates at `3aa0371`**: `bunx tsc --noEmit` **81** errors; `bunx vitest run` **98
  pass / 24 fail** (the 24 = documented jsdom/localStorage race in `draft-storage` +
  `imported-columns`, unrelated to reports UI); `bun run build` exit 0.

### Considered and de-prioritized (Round 7)

- **Hand-rolled metric cards** at `reports.index.tsx:473–492` and the whole
  `holistic-reports.index.tsx` route — real (Title-case labels, `rounded-lg border bg-card`
  divs), but these live in the **legacy / non-pipeline branch** and a **stale near-duplicate
  route**; the live P1-A cycle hub already dropped the summary cards (plans 024/026). Folded
  into 028/029 as low-priority same-file edits rather than their own plan.
- **`send-to-parents-dialog.tsx` native radios** — no `ui/RadioGroup` exists; creating one
  is out of scope (component-creation) and the site is otherwise cleanly composed. Left alone,
  noted in 028's STOP conditions.
- **Dark-mode breakage from `bg-white`/default-slate** — real, but the dark-mode toggle is
  still unwired (documented deferred item), so it's latent, not live. 027 removes the literals
