...
function Checkbox({
  ...
  // Base UI defaults to a <span role="checkbox">. Opt in to a real <button>
  // only where the checkbox is a standalone control: several existing call
  // sites nest <Checkbox> inside another <button> (decorative), where a
  // native button would emit invalid <button><button> markup.
  const nativeButtonProps = nativeButton
    ? { nativeButton: true as const, render: <button type="button" /> }
    : {}
  return (
